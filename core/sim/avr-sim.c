 ...
 */
void copy_options_to_avr_env(avr_env_t *env, init_options *opt)
{
	env->argc = opt->argc;
...
FILE* load_profiling_file(char* profiling_file_name, int inst_stat[])
{
    ...
    if(profile_id == NULL)
    {
        ...
        for(i = 0; i<AVR_INSTRUCTIONS_NB; i++)
...
void write_profiling_file(FILE* profile_id, int inst_stat[])
{
	...
	}tmp, entries[AVR_INSTRUCTIONS_NB];
	...
	for(i = 0; i<AVR_INSTRUCTIONS_NB; i++)
	{
		for(j = i+1; j<AVR_INSTRUCTIONS_NB; j++)
		{
    ...
    for(i = 0; i<AVR_INSTRUCTIONS_NB; i++)
    {
        fprintf(profile_id, "%s %d %d\n",
        	avr_get_string_ident(entries[i].id),	// instruction name
...
int main(int argc, char **argv) {
    avr_state_t *state = 0;
    avr_platform_t *platform = 0;
    avr_loader_t *loader = 0;
    avr_address_t addr_start = 0;
    avr_address_t addr_exit = 0;
	char *c_ptr = 0;
    char *profiling_file_name;
    char *profiling_file_path = NULL;
    FILE *profile_id=0;
    int inst_stat[AVR_INSTRUCTIONS_NB];
	...
	/* open the exec file */
    loader = avr_loader_open(argv[prog_index]);
	if(loader == NULL) {
	...
	if(!is_start_given)
        addr_start = avr_loader_start(loader);
	if(verbose)
		printf("START=%08x\n", addr_start);
	...
	if (!is_exit_given) {
		...
		/* search symbol _exit */
        for(sym_exit = 0; sym_exit < avr_loader_count_syms(loader); sym_exit++) {
            avr_loader_sym_t data;
            avr_loader_sym(loader, sym_exit, &data);
    ...
    copy_options_to_avr_env(avr_get_sys_env(platform), &options);

	/* load the image in the platform */
    if (avr_load_platform(platform, argv[prog_index]) == -1) {
	...
	/* make the simulator */
    sim = avr_new_sim(state, addr_start, addr_exit);
	if (sim == NULL) {
    ...
    if(profile)
    {
        ...
        if( profiling_file_path != NULL){
			profiling_file_name = (char*)alloca(sizeof(char)* (strlen(profiling_file_path) + strlen(AVR_PROC_NAME) + 10));
			...
			strcat(profiling_file_name, AVR_PROC_NAME);
		}else{
			profiling_file_name = (char*)alloca(sizeof(char)* strlen(AVR_PROC_NAME) + 10);
			strcpy(profiling_file_name, AVR_PROC_NAME);
		}
...
#	endif
    ...
    if(!verbose && !profile && !valid)
    {
			...
			if(fast_sim)
				inst_cnt += avr_run_and_count_inst(sim);
			else
			{
				while(addr_exit != state->AVR_PC_NAME
				&& !avr_is_sim_ended(sim))
				{
					avr_step(sim);
					inst_cnt++;
				}
	...
	else
	{
        ...
        if(valid) {
			...
			avr_output_header_valid(vout);
		}

		/* perform the simulation */
        while(!avr_is_sim_ended(sim)) {
            inst = avr_next_inst(sim);
            if( profile )
                inst_stat[inst->ident]++;
            if(verbose) {
                avr_disasm(buffer, inst);
                fprintf(stderr, "%08x: %s\n", avr_next_addr(sim),  buffer);
            }
            if(valid)
				avr_output_state_valid(state, vout);
            avr_free_inst(inst);
            avr_step(sim);
            if(dump) {
