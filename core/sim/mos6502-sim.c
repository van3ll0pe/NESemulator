 ...
 */
void copy_options_to_mos6502_env(mos6502_env_t *env, init_options *opt)
{
	env->argc = opt->argc;
...
FILE* load_profiling_file(char* profiling_file_name, int inst_stat[])
{
    ...
    if(profile_id == NULL)
    {
        ...
        for(i = 0; i<MOS6502_INSTRUCTIONS_NB; i++)
...
void write_profiling_file(FILE* profile_id, int inst_stat[])
{
	...
	}tmp, entries[MOS6502_INSTRUCTIONS_NB];
	...
	for(i = 0; i<MOS6502_INSTRUCTIONS_NB; i++)
	{
		for(j = i+1; j<MOS6502_INSTRUCTIONS_NB; j++)
		{
    ...
    for(i = 0; i<MOS6502_INSTRUCTIONS_NB; i++)
    {
        fprintf(profile_id, "%s %d %d\n",
        	mos6502_get_string_ident(entries[i].id),	// instruction name
...
int main(int argc, char **argv) {
    mos6502_state_t *state = 0;
    mos6502_platform_t *platform = 0;
    mos6502_loader_t *loader = 0;
    mos6502_address_t addr_start = 0;
    mos6502_address_t addr_exit = 0;
	char *c_ptr = 0;
    char *profiling_file_name;
    char *profiling_file_path = NULL;
    FILE *profile_id=0;
    int inst_stat[MOS6502_INSTRUCTIONS_NB];
	...
	/* open the exec file */
    loader = mos6502_loader_open(argv[prog_index]);
	if(loader == NULL) {
	...
	if(!is_start_given)
        addr_start = mos6502_loader_start(loader);
	if(verbose)
		printf("START=%08x\n", addr_start);
	...
	if (!is_exit_given) {
		...
		/* search symbol _exit */
        for(sym_exit = 0; sym_exit < mos6502_loader_count_syms(loader); sym_exit++) {
            mos6502_loader_sym_t data;
            mos6502_loader_sym(loader, sym_exit, &data);
    ...
    copy_options_to_mos6502_env(mos6502_get_sys_env(platform), &options);

	/* load the image in the platform */
    if (mos6502_load_platform(platform, argv[prog_index]) == -1) {
    ...
    state = mos6502_new_state(platform);
	if (state == NULL)  {
	...
	/* make the simulator */
    sim = mos6502_new_sim(state, addr_start, addr_exit);
	if (sim == NULL) {
    ...
    if(profile)
    {
        ...
        if( profiling_file_path != NULL){
			profiling_file_name = (char*)alloca(sizeof(char)* (strlen(profiling_file_path) + strlen(MOS6502_PROC_NAME) + 10));
			...
			strcat(profiling_file_name, MOS6502_PROC_NAME);
		}else{
			profiling_file_name = (char*)alloca(sizeof(char)* strlen(MOS6502_PROC_NAME) + 10);
			strcpy(profiling_file_name, MOS6502_PROC_NAME);
		}
...
#	endif
    ...
    if(!verbose && !profile && !valid)
    {
			...
			if(fast_sim)
				inst_cnt += mos6502_run_and_count_inst(sim);
			else
			{
				while(addr_exit != state->MOS6502_PC_NAME
				&& !mos6502_is_sim_ended(sim))
				{
					mos6502_step(sim);
					inst_cnt++;
				}
	...
	else
	{
        ...
        if(valid) {
			...
			mos6502_output_header_valid(vout);
		}

		/* perform the simulation */
        while(!mos6502_is_sim_ended(sim)) {
            inst = mos6502_next_inst(sim);
            if( profile )
                inst_stat[inst->ident]++;
            if(verbose) {
                mos6502_disasm(buffer, inst);
                fprintf(stderr, "%08x: %s\n", mos6502_next_addr(sim),  buffer);
            }
            if(valid)
				mos6502_output_state_valid(state, vout);
