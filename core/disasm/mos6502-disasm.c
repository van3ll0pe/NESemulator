 ...
 */
void add_to_list(list_entry_t **m, const char *n, mos6502_address_t a) {

#	ifdef MOS6502_PROCESS_CODE_LABEL
		{ MOS6502_PROCESS_CODE_LABEL(a); }
#	endif
...
*/
int get_label_from_list(list_entry_t *m, mos6502_address_t addr, const char **name) {
...
*/
list_entry_t *get_closer_label_from_list(list_entry_t *m, mos6502_address_t addr) {
...
static void fail_with_help(const char *msg, ...) {
	...
	if(mos6502_modes[0].name) {
		...
		for(i = 0; mos6502_modes[i].name; i++) {
			if(!fst)
				fprintf(stderr, ", ");
			else
				fst = 0;
			fputs(mos6502_modes[i].name, stderr);
		}
		fprintf(stderr, "\n");
	}
 ...
 */
char *mos6502_solve_label_disasm(mos6502_address_t address) {
	static char buf[256];
...
int main(int argc, char **argv) {
	...
	int s_it;
	mos6502_loader_sect_t *s_tab;
	int sym_it;
	int nb_sect_disasm = 0;
	mos6502_loader_t *loader;
	int max_size = 0, i, j;
	char *exe_path = 0;
	mos6502_inst_t *(*decode)(mos6502_decoder_t *decoder, mos6502_address_t address) = mos6502_decode;
	int i_sect;
	mos6502_decoder_t *d;

	/* test arguments */
	for(i = 1; i < argc; i++) {
		if(mos6502_modes[0].name && strcmp(argv[i], "-m") == 0) {
			i++;
			if(i >= argc)
				fail_with_help("no argument for -m option");
			for(j = 0; mos6502_modes[j].name; j++)
				if(strcmp(argv[i], mos6502_modes[j].name) == 0) {
					decode = mos6502_modes[j].decode;
					break;
				}
			if(!mos6502_modes[j].name)
	...
	loader = mos6502_loader_open(exe_path);
	if (loader == NULL) {
	...
	/* display sections */
	printf("found %d sections in the executable %s\n", mos6502_loader_count_sects(loader)-1, exe_path);
	s_tab = (mos6502_loader_sect_t *)malloc(mos6502_loader_count_sects(loader) * sizeof(mos6502_loader_sect_t));
	for(s_it = 0; s_it < mos6502_loader_count_sects(loader); s_it++) {
		mos6502_loader_sect_t data;
		mos6502_loader_sect(loader, s_it, &data);
		if(data.type == MOS6502_LOADER_SECT_TEXT) {
	...
	/* display symbols */
	printf("\nfound %d symbols in the executable %s\n", mos6502_loader_count_syms(loader)-1, exe_path);
	for(sym_it = 0; sym_it < mos6502_loader_count_syms(loader); sym_it++) {
		mos6502_loader_sym_t data;
		mos6502_loader_sym(loader, sym_it, &data);
		if(data.type == MOS6502_LOADER_SYM_CODE || data.type == MOS6502_LOADER_SYM_DATA) {
			printf("[L]");
			if(data.sect != 0)
	...
	mos6502_solve_label = mos6502_solve_label_disasm;

	/* create the platform */
	pf = mos6502_new_platform();
	if(pf == NULL) {
	...
	/* load it */
	mos6502_loader_load(loader, pf);

	d = mos6502_new_decoder(pf);
	/* multi iss part, TODO: improve */
	mos6502_state_t *state = mos6502_new_state(pf);
	...
	mos6502_set_cond_state(d, state);

	/* compute instruction max size */
	for(i = 1; i < MOS6502_TOP; i++) {
		int size = mos6502_get_inst_size_from_id(i) / 8;
		if(size > max_size)
			max_size = size;
	}
	...
	for(i_sect = 0; i_sect<nb_sect_disasm; i_sect++) {
		...
		/* display new section */
		mos6502_address_t adr_start = s_tab[i_sect].addr;
		mos6502_address_t adr_end = s_tab[i_sect].addr + s_tab[i_sect].size;
		mos6502_address_t prev_addr = 0;
		...
		while (adr_start < adr_end) {
			...
			int size;
			char buff[100];
			mos6502_inst_t *inst = decode(d, adr_start);
			mos6502_disasm(buff, inst);
			const char *n;
			...
			size = mos6502_get_inst_size(inst) / 8;
			for(i = 0; i < max_size; i++) {
				if(i < size)
					printf("%02x", mos6502_mem_read8(mos6502_get_memory(pf, 0), adr_start + i));
				else
					fputs("  ", stdout);
			}
	...
	/* cleanup */
	mos6502_delete_decoder(d);
	mos6502_unlock_platform(pf);
	destroy_list(labels);
