 ...
 */
void add_to_list(list_entry_t **m, const char *n, avr_address_t a) {

#	ifdef AVR_PROCESS_CODE_LABEL
		{ AVR_PROCESS_CODE_LABEL(a); }
#	endif
...
*/
int get_label_from_list(list_entry_t *m, avr_address_t addr, const char **name) {
...
*/
list_entry_t *get_closer_label_from_list(list_entry_t *m, avr_address_t addr) {
...
static void fail_with_help(const char *msg, ...) {
	...
	if(avr_modes[0].name) {
		...
		for(i = 0; avr_modes[i].name; i++) {
			if(!fst)
				fprintf(stderr, ", ");
			else
				fst = 0;
			fputs(avr_modes[i].name, stderr);
		}
		fprintf(stderr, "\n");
	}
 ...
 */
char *avr_solve_label_disasm(avr_address_t address) {
	static char buf[256];
...
int main(int argc, char **argv) {
	avr_platform_t *pf;
	int s_it;
	avr_loader_sect_t *s_tab;
	int sym_it;
	int nb_sect_disasm = 0;
	avr_loader_t *loader;
	int max_size = 0, i, j;
	char *exe_path = 0;
	avr_inst_t *(*decode)(avr_decoder_t *decoder, avr_address_t address) = avr_decode;
	int i_sect;
	avr_decoder_t *d;

	/* test arguments */
	for(i = 1; i < argc; i++) {
		if(avr_modes[0].name && strcmp(argv[i], "-m") == 0) {
			i++;
			if(i >= argc)
				fail_with_help("no argument for -m option");
			for(j = 0; avr_modes[j].name; j++)
				if(strcmp(argv[i], avr_modes[j].name) == 0) {
					decode = avr_modes[j].decode;
					break;
				}
			if(!avr_modes[j].name)
	...
	loader = avr_loader_open(exe_path);
	if (loader == NULL) {
	...
	/* display sections */
	printf("found %d sections in the executable %s\n", avr_loader_count_sects(loader)-1, exe_path);
	s_tab = (avr_loader_sect_t *)malloc(avr_loader_count_sects(loader) * sizeof(avr_loader_sect_t));
	for(s_it = 0; s_it < avr_loader_count_sects(loader); s_it++) {
		avr_loader_sect_t data;
		avr_loader_sect(loader, s_it, &data);
		if(data.type == AVR_LOADER_SECT_TEXT) {
	...
	/* display symbols */
	printf("\nfound %d symbols in the executable %s\n", avr_loader_count_syms(loader)-1, exe_path);
	for(sym_it = 0; sym_it < avr_loader_count_syms(loader); sym_it++) {
		avr_loader_sym_t data;
		avr_loader_sym(loader, sym_it, &data);
		if(data.type == AVR_LOADER_SYM_CODE || data.type == AVR_LOADER_SYM_DATA) {
			printf("[L]");
			if(data.sect != 0)
	...
	avr_solve_label = avr_solve_label_disasm;
	...
	/* load it */
	avr_loader_load(loader, pf);

	d = avr_new_decoder(pf);
	/* multi iss part, TODO: improve */
	avr_state_t *state = avr_new_state(pf);
	...
	avr_set_cond_state(d, state);

	/* compute instruction max size */
	for(i = 1; i < AVR_TOP; i++) {
		int size = avr_get_inst_size_from_id(i) / 8;
		if(size > max_size)
			max_size = size;
	}
	...
	for(i_sect = 0; i_sect<nb_sect_disasm; i_sect++) {
		...
		/* display new section */
		avr_address_t adr_start = s_tab[i_sect].addr;
		avr_address_t adr_end = s_tab[i_sect].addr + s_tab[i_sect].size;
		avr_address_t prev_addr = 0;
		...
		while (adr_start < adr_end) {
			...
			int size;
			char buff[100];
			avr_inst_t *inst = decode(d, adr_start);
			avr_disasm(buff, inst);
			const char *n;
			...
			size = avr_get_inst_size(inst) / 8;
			for(i = 0; i < max_size; i++) {
				if(i < size)
					printf("%02x", avr_mem_read8(avr_get_memory(pf, 0), adr_start + i));
				else
					fputs("  ", stdout);
			}
	...
	/* cleanup */
	avr_delete_decoder(d);
	avr_unlock_platform(pf);
	destroy_list(labels);
