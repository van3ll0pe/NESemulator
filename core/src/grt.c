 ...
 */
uint64_t mos6502_d2bits(double d) {
...
{
	uint32_t mask = mos6502_mask32(l);
	if (l == 32)
		r = FMOD(r, 32);
	else {
		r = r % l;	// slow modulo
...
uint32_t mos6502_rotate_left8(int l, uint32_t v, int r)
{
	...
	/*r = FMOD(r, 8);
	int len = 8 - r;
	uint32_t mask = mos6502_mask32(len);
...
{
	uint64_t mask = mos6502_mask64(l);
	if (l == 64)
		r = FMOD(r, 64);
	else {
		r = r % l;	// slow modulo
	...
	/*r = FMOD(r, 64);
	int len = 64 - r;
	uint64_t mask = mos6502_mask64(len);
	return (((v & mask) << r) | ((v & ~mask) >> len));*/
}
...
uint32_t mos6502_rotate_right32(int l, uint32_t v, int r)
{
	...
	/*r = FMOD(r, 32);
	uint32_t mask = mos6502_mask32(r);
...
uint64_t mos6502_rotate_right64(int l, uint64_t v, int r)
{
	...
	return ((v >> r) | (v << (l - r))) & mask;
	/*r = FMOD(r, 64);
	uint64_t mask = mos6502_mask64(r);
...
uint32_t mos6502_invert32(uint32_t v, uint32_t n)
{
	uint32_t res = 0;
	int i = 0;

	/* !!DEBUG!! */
	//printf("mos6502_invert32(0X%08X, 0X%08X) => ", v, n);
...
uint64_t mos6502_invert64(uint64_t v, uint64_t n)
{
	uint64_t res = 0;
	int i = 0;

	/* !!DEBUG!! */
	//printf("mos6502_invert64(0X%016LX, 0X%016LX) => ", v, n);
...
float mos6502_invertf(float v, uint32_t n)
{
	*((uint32_t *)&v) = mos6502_invert32(*(uint32_t *)&v, n);
	return v;
}

double mos6502_invertd(double v, uint32_t n)
{
	*((uint64_t *)&v) = mos6502_invert64(*(uint64_t *)&v, n);
	return v;
}

/* for these functions no inversion is done and l <= u */
#ifdef COMPAT
uint32_t mos6502_set_field32u(uint32_t v, uint32_t s, int32_t u, int32_t l) {
...
uint64_t mos6502_set_field64u(uint64_t v, uint64_t s, int32_t u, int32_t l) {
	uint64_t mask = mos6502_mask64(u - l + 1) << l;

	/* !!DEBUG!! */
	//printf("mos6502_set_field64u(0X%016llX, 0X%016llX, 0X%08X, 0X%08X) => ", v, s, u, l);
	//printf("0X%016llX\n", (v & ~mask) | ((s << l) & mask));
...
float mos6502_set_fieldf(float v, uint32_t s, int32_t u, int32_t l) {
	...
	x.i = mos6502_set_field32u(*((uint32_t *)&v), s, u, l);
	return x.d;
}
...
double mos6502_set_fieldd(double v, uint64_t s, int32_t u, int32_t l) {
	...
	x.i = mos6502_set_field64u(*((uint64_t *)&v), s, u, l);
	return x.d;
}
...
uint32_t mos6502_set_field32u_inverted(uint32_t v, uint32_t s, int32_t u, int32_t l) {
	uint32_t mask = mos6502_mask32(u - l + 1);
	assert(u >= l);

	/* !!DEBUG!! */
	//printf("mos6502_set_field32u_inverted(0X%08X, 0X%08X, 0X%08X, 0X%08X) => ", v, s, u, l);
	//printf("0X%08X\n", (v & ~mask) | (mos6502_invert32(s & mask, u-l+1) << l));

	return (v & ~(mask << l)) | (mos6502_invert32(s, u-l+1) << l);
}

uint64_t mos6502_set_field64u_inverted(uint64_t v, uint64_t s, int32_t u, int32_t l) {
	uint64_t mask = mos6502_mask64(u - l + 1);

	/* !!DEBUG!! */
	//printf("mos6502_set_field64u_inverted(0X%016llX, 0X%016llX, 0X%08X, 0X%08X) => ", v, s, u, l);
	//printf("0X%016llX\n", (v & ~mask) | (mos6502_invert64(s & mask, u-l+1) << l));

	return (v & ~(mask << l)) | (mos6502_invert64(s, u-l+1) << l);
}

float mos6502_set_fieldf_inverted(float v, uint32_t s, int32_t u, int32_t l) {
	*((uint32_t *)&v) = mos6502_set_field32u_inverted(*((uint32_t *)&v), s, u, l);
	return v;
}

double mos6502_set_fieldd_inverted(double v, uint64_t s, int32_t u, int32_t l) {
	*((uint64_t *)&v) = mos6502_set_field64u_inverted(*((uint64_t *)&v), s, u, l);
	return v;
}
...
{
	/* !!DEBUG!! */
	//printf("mos6502_set_field32u_generic(0X%08X, 0X%08X, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, s, a, b, bit_order);

	/* only 1 bit to copy, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return mos6502_set_field32u(v, s, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return mos6502_set_field32u_inverted(v, s, b, a);
		else
			return mos6502_set_field32u(v, s, b, a);
	}
	else
	{
		if (bit_order)
			...
			return mos6502_set_field32_inverted(v, s, a, b);
	}
}
...
{
	/* !!DEBUG!! */
	//printf("mos6502_set_field64u_generic(0X%016llX, 0X%016llX, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, s, a, b, bit_order);

	/* only 1 bit to copy, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return mos6502_set_field64u(v, s, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return mos6502_set_field64u_inverted(v, s, b, a);
		else
			return mos6502_set_field64u(v, s, b, a);
	}
	else
	{
		if (bit_order)
			...
			return mos6502_set_field64_inverted(v, s, a, b);
	}
}
...
float mos6502_set_fieldf_generic(float v, uint32_t s, int32_t a, int32_t b, int bit_order) {
	...
	x.i = mos6502_set_field32u_generic(*((uint32_t *)&v), s, a, b, bit_order);
	return x.f;
}
...
double mos6502_set_fieldd_generic(double v, uint64_t s, int32_t a, int32_t b, int bit_order) {
	...
	x.i = mos6502_set_field64u_generic(*(uint64_t *)&v, s, a, b, bit_order);
	return x.d;
}
...
uint32_t mos6502_field32u_inverted(uint32_t v, uint32_t u, uint32_t l)
{
	...
	/* !!DEBUG!! */
	//printf("mos6502_field32u_inverted(0X%08X, 0X%08X, 0X%08X) => ", v, u, l);
	//printf("0X%08X\n", mos6502_invert32((v & mask) >> l, u-l+1));

	return mos6502_invert32((v & mask) >> l, u-l+1);
}
...
uint64_t mos6502_field64u_inverted(uint64_t v, uint32_t u, uint32_t l)
{
	...
	/* !!DEBUG!! */
	//printf("mos6502_field64u_inverted(0X%016LX, 0X%08X, 0X%08X) => ", v, u, l);
	//printf("0X%016LX\n", mos6502_invert64((v & mask) >> l, u-l+1));

	return mos6502_invert64((v & mask) >> l, u-l+1);
}


uint32_t mos6502_fieldf_inverted(float v, uint32_t u, uint32_t l) {
	return mos6502_field32u_inverted(*(uint32_t *)&v, u, l);
}

uint64_t mos6502_fieldd_inverted(double v, uint32_t u, uint32_t l) {
	return mos6502_field64u_inverted(*(uint64_t *)&v, u, l);
}
...
{
	/* !!DEBUG!! */
	//printf("mos6502_field32u_generic(0X%08X, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, a, b, bit_order);

	/* only 1 bit to extract, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return mos6502_field32u(v, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return mos6502_field32u_inverted(v, b, a);
		else
			return mos6502_field32u(v, b, a);
	}
	else
	{
		if (bit_order)
			...
			return mos6502_field32_inverted(v, a, b);
	}
}
...
{
	/* !!DEBUG!! */
	//printf("mos6502_field64u_generic(0X%016LX, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, a, b, bit_order);

	/* only 1 bit to extract, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return mos6502_field64u(v, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return mos6502_field64u_inverted(v, b, a);
		else
			return mos6502_field64u(v, b, a);
	}
	else
	{
		if (bit_order)
...
int mos6502_enumerate(int val, int max) {
	if(val >= max)
		mos6502_panic("coercition to enumeration out of bounds");
	else
		return val;
}
 ...
 * @return	Double value.
 */
double mos6502_cast_64tod(uint64_t i) {
	union {
		uint64_t i;
		double d;
	} x;
