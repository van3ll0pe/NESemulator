...
uint32_t avr_invert32(uint32_t v, uint32_t n)
{
	uint32_t res = 0;
	int i = 0;

	/* !!DEBUG!! */
	//printf("avr_invert32(0X%08X, 0X%08X) => ", v, n);
...
uint64_t avr_invert64(uint64_t v, uint64_t n)
{
	uint64_t res = 0;
	int i = 0;

	/* !!DEBUG!! */
	//printf("avr_invert64(0X%016LX, 0X%016LX) => ", v, n);
...
float avr_invertf(float v, uint32_t n)
{
	*((uint32_t *)&v) = avr_invert32(*(uint32_t *)&v, n);
	return v;
}

double avr_invertd(double v, uint32_t n)
{
	*((uint64_t *)&v) = avr_invert64(*(uint64_t *)&v, n);
	return v;
}

/* for these functions no inversion is done and l <= u */
#ifdef COMPAT
uint32_t avr_set_field32u(uint32_t v, uint32_t s, int32_t u, int32_t l) {
...
uint64_t avr_set_field64u(uint64_t v, uint64_t s, int32_t u, int32_t l) {
	uint64_t mask = avr_mask64(u - l + 1) << l;

	/* !!DEBUG!! */
	//printf("avr_set_field64u(0X%016llX, 0X%016llX, 0X%08X, 0X%08X) => ", v, s, u, l);
	//printf("0X%016llX\n", (v & ~mask) | ((s << l) & mask));
...
float avr_set_fieldf(float v, uint32_t s, int32_t u, int32_t l) {
	...
	x.i = avr_set_field32u(*((uint32_t *)&v), s, u, l);
	return x.d;
}
...
double avr_set_fieldd(double v, uint64_t s, int32_t u, int32_t l) {
	...
	x.i = avr_set_field64u(*((uint64_t *)&v), s, u, l);
	return x.d;
}
...
uint32_t avr_set_field32u_inverted(uint32_t v, uint32_t s, int32_t u, int32_t l) {
	uint32_t mask = avr_mask32(u - l + 1);
	assert(u >= l);

	/* !!DEBUG!! */
	//printf("avr_set_field32u_inverted(0X%08X, 0X%08X, 0X%08X, 0X%08X) => ", v, s, u, l);
	//printf("0X%08X\n", (v & ~mask) | (avr_invert32(s & mask, u-l+1) << l));

	return (v & ~(mask << l)) | (avr_invert32(s, u-l+1) << l);
}

uint64_t avr_set_field64u_inverted(uint64_t v, uint64_t s, int32_t u, int32_t l) {
	uint64_t mask = avr_mask64(u - l + 1);

	/* !!DEBUG!! */
	//printf("avr_set_field64u_inverted(0X%016llX, 0X%016llX, 0X%08X, 0X%08X) => ", v, s, u, l);
	//printf("0X%016llX\n", (v & ~mask) | (avr_invert64(s & mask, u-l+1) << l));

	return (v & ~(mask << l)) | (avr_invert64(s, u-l+1) << l);
}

float avr_set_fieldf_inverted(float v, uint32_t s, int32_t u, int32_t l) {
	*((uint32_t *)&v) = avr_set_field32u_inverted(*((uint32_t *)&v), s, u, l);
	return v;
}

double avr_set_fieldd_inverted(double v, uint64_t s, int32_t u, int32_t l) {
	*((uint64_t *)&v) = avr_set_field64u_inverted(*((uint64_t *)&v), s, u, l);
	return v;
}
...
{
	/* !!DEBUG!! */
	//printf("avr_set_field32u_generic(0X%08X, 0X%08X, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, s, a, b, bit_order);

	/* only 1 bit to copy, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return avr_set_field32u(v, s, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return avr_set_field32u_inverted(v, s, b, a);
		else
			return avr_set_field32u(v, s, b, a);
	}
	else
	{
		if (bit_order)
			...
			return avr_set_field32_inverted(v, s, a, b);
	}
}
...
{
	/* !!DEBUG!! */
	//printf("avr_set_field64u_generic(0X%016llX, 0X%016llX, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, s, a, b, bit_order);

	/* only 1 bit to copy, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return avr_set_field64u(v, s, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return avr_set_field64u_inverted(v, s, b, a);
		else
			return avr_set_field64u(v, s, b, a);
	}
	else
	{
		if (bit_order)
			...
			return avr_set_field64_inverted(v, s, a, b);
	}
}
...
float avr_set_fieldf_generic(float v, uint32_t s, int32_t a, int32_t b, int bit_order) {
	...
	x.i = avr_set_field32u_generic(*((uint32_t *)&v), s, a, b, bit_order);
	return x.f;
}
...
double avr_set_fieldd_generic(double v, uint64_t s, int32_t a, int32_t b, int bit_order) {
	...
	x.i = avr_set_field64u_generic(*(uint64_t *)&v, s, a, b, bit_order);
	return x.d;
}
...
uint32_t avr_field32u_inverted(uint32_t v, uint32_t u, uint32_t l)
{
	...
	/* !!DEBUG!! */
	//printf("avr_field32u_inverted(0X%08X, 0X%08X, 0X%08X) => ", v, u, l);
	//printf("0X%08X\n", avr_invert32((v & mask) >> l, u-l+1));

	return avr_invert32((v & mask) >> l, u-l+1);
}
...
uint64_t avr_field64u_inverted(uint64_t v, uint32_t u, uint32_t l)
{
	...
	/* !!DEBUG!! */
	//printf("avr_field64u_inverted(0X%016LX, 0X%08X, 0X%08X) => ", v, u, l);
	//printf("0X%016LX\n", avr_invert64((v & mask) >> l, u-l+1));

	return avr_invert64((v & mask) >> l, u-l+1);
}


uint32_t avr_fieldf_inverted(float v, uint32_t u, uint32_t l) {
	return avr_field32u_inverted(*(uint32_t *)&v, u, l);
}

uint64_t avr_fieldd_inverted(double v, uint32_t u, uint32_t l) {
	return avr_field64u_inverted(*(uint64_t *)&v, u, l);
}
...
{
	/* !!DEBUG!! */
	//printf("avr_field32u_generic(0X%08X, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, a, b, bit_order);

	/* only 1 bit to extract, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return avr_field32u(v, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return avr_field32u_inverted(v, b, a);
		else
			return avr_field32u(v, b, a);
	}
	else
	{
		if (bit_order)
			...
			return avr_field32_inverted(v, a, b);
	}
}
...
{
	/* !!DEBUG!! */
	//printf("avr_field64u_generic(0X%016LX, 0X%08X, 0X%08X, %d) ==> \\\\\\\n++++", v, a, b, bit_order);

	/* only 1 bit to extract, do not care about bit_order, just do it the most convenient way */
	if (a == b)
		return avr_field64u(v, a, b);

	/* lower bits first and uppermost OR upper bits first and lowermost => inversion */
	if (a < b)
	{
		if (bit_order)
			return avr_field64u_inverted(v, b, a);
		else
			return avr_field64u(v, b, a);
	}
	else
	{
		if (bit_order)
...
int avr_enumerate(int val, int max) {
	if(val >= max)
		avr_panic("coercition to enumeration out of bounds");
	else
		return val;
}
 ...
 * @return	Double value.
 */
double avr_cast_64tod(uint64_t i) {
	union {
		uint64_t i;
		double d;
	} x;
