...
//#define MOS6502_MEM_SPY		// define it to get memory action spy feature
...
/**
 * @typedef mos6502_memory_t
 * This type is used to represent a memory space.
 * @ingroup memory
 */
typedef struct mos6502_memory_t
{
...
#	endif
	...
	mos6502_mem_spy_t spy_fun;	/** spy function */
 ...
 */
static void mos6502_mem_default_spy(mos6502_memory_t *mem, mos6502_address_t addr, mos6502_size_t size, mos6502_access_t access, void *data) {
}
#endif
...
mos6502_memory_t* mos6502_mem_new(void) {
	...
	/* initialize spy */
#	ifdef MOS6502_MEM_SPY
		mem->spy_fun = mos6502_mem_default_spy;
		mem->spy_data = 0;
#	endif

	return (mos6502_memory_t*) mem;
}
 ...
 */
static page_entry_t* mem_get_page(memory_64_t* mem, mos6502_address_t addr)
{
    uint32_t      hash1;
    page_entry_t* tmp;
    page_entry_t* entry;
...
mos6502_memory_t* mos6502_mem_copy(mos6502_memory_t* memory)
{
    ...
    memory_64_t* target = (memory_64_t*)mos6502_mem_new();
 ...
 * @ingroup memory
 */
void mos6502_mem_write(mos6502_memory_t* memory, mos6502_address_t address, void* buffer, size_t size)
{
    assert(size > 0);
	int i;
...
#	ifdef MOS6502_MEM_SPY
    	mem->spy_fun(mem, address, size, mos6502_access_write, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
void mos6502_mem_read(mos6502_memory_t *memory, mos6502_address_t address, void *buffer, size_t size)//////////////////////////////
{
    assert(size > 0);
	int i;
    uint32_t      offset = FMOD(address, MEM_PAGE_SIZE);
...
#	ifdef MOS6502_MEM_SPY
    	mem->spy_fun(mem, address, size, mos6502_access_read, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
uint8_t mos6502_mem_read8(mos6502_memory_t *mem, mos6502_address_t address)
{
    page_entry_t* pte    = mem_get_page(mem, address);
    mos6502_address_t offset = FMOD(address, MEM_PAGE_SIZE);
    uint8_t r;
...
#	ifdef MOS6502_MEM_SPY
    	mem->spy_fun(mem, address, sizeof(r), mos6502_access_read, mem->spy_data);
#	endif
    return r;
}
 ...
 * @ingroup memory
 */
uint16_t mos6502_mem_read16(mos6502_memory_t *mem, mos6502_address_t address) {
...
#   endif
    ...
    if(!((offset & 0x1) | ((offset + 1) & MEM_PAGE_SIZE)))
        ...
        mos6502_mem_read(mem, address, val.bytes, 2);
        r = val.half;
    }
 ...
 * @ingroup memory
 */
uint32_t mos6502_mem_read32(mos6502_memory_t *mem, mos6502_address_t address)
{
...
#   endif
    ...
    else
    {
		...
		mos6502_mem_read(mem, address, val.bytes, 4);
        r = val.word;
    }
 ...
 * @ingroup memory
 */
uint64_t mos6502_mem_read64(mos6502_memory_t *mem, mos6502_address_t address) {
...
#   if HOST_ENDIANNESS != TARGET_ENDIANNESS
    ...
    else
    {
        ...
        mos6502_mem_read(mem, address, val.bytes, 8);
        r = val.dword;
    }
 ...
 * @ingroup memory
 */
float mos6502_mem_readf(mos6502_memory_t *mem, mos6502_address_t address)
{
	union {
		uint32_t i;
		float f;
	} val;
    val.i = mos6502_mem_read32(mem, address);
	return val.f;
}
 ...
 * @ingroup memory
 */
double mos6502_mem_readd(mos6502_memory_t *mem, mos6502_address_t address)
{
	union {
		uint64_t i;
		double f;
	} val;
    val.i = mos6502_mem_read64(mem, address);
	return val.f;
}
 ...
 * @ingroup memory
 */
long double mos6502_mem_readld(mos6502_memory_t *memory, mos6502_address_t address)
{
 ...
 * @ingroup memory
 */
void mos6502_mem_write8(mos6502_memory_t* mem, mos6502_address_t address, uint8_t val)
{
    mos6502_address_t offset;
    page_entry_t*   pte;
    pte    = mem_get_page(mem, address);
...
#	ifdef MOS6502_MEM_SPY
    	mem->spy_fun(mem, address, sizeof(val), mos6502_access_write, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
void mos6502_mem_write16(mos6502_memory_t* mem, mos6502_address_t address, uint16_t val)
{
    mos6502_address_t offset;
    uint16_t* q;
...
# endif
    ...
    if(!((offset & 0x1) | ((offset + 1) & MEM_PAGE_SIZE)))
        *q = val;
		...
		mos6502_mem_write(mem, address, p->bytes, 2);
	}
 ...
 * @ingroup memory
 */
void mos6502_mem_write32(mos6502_memory_t* mem, mos6502_address_t address, uint32_t val) {
	uint32_t* q;
    mos6502_address_t offset;
    page_entry_t*   pte;
...
#   endif
    ...
    if(!((offset & 0x3) | ((offset + 3) & MEM_PAGE_SIZE)))
        *q = val;
        ...
        mos6502_mem_write(mem, address, p->bytes, 4);
	}
 ...
 * @ingroup memory
 */
void mos6502_mem_write64(mos6502_memory_t *mem, mos6502_address_t address, uint64_t val)
{
    mos6502_address_t offset;
    uint64_t* q;
...
#   endif
    ...
    if(!((offset & 0x7) | ((offset + 7) & MEM_PAGE_SIZE)))
        *q = val;
        ...
        mos6502_mem_write(mem, address, p->bytes, 8);
	}
 ...
 * @ingroup memory
 */
void mos6502_mem_writef(mos6502_memory_t *memory, mos6502_address_t address, float val)
{
	union {
		uint32_t i;
		float f;
	...
	v.f = val;
    mos6502_mem_write32(memory, address, v.i);
}
 ...
 * @ingroup memory
 */
void mos6502_mem_writed(mos6502_memory_t *memory, mos6502_address_t address, double val)
{
	union {
		uint64_t i;
		double f;
	...
	v.f = val;
    mos6502_mem_write64(memory, address, v.i);
}
 ...
 * @ingroup memory
 */
void mos6502_mem_writeld(mos6502_memory_t *memory, mos6502_address_t address, long double val)
{
	assertp(0, "not implemented");
}
