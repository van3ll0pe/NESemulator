...
//#define AVR_MEM_SPY		// define it to get memory action spy feature
...
/**
 * @typedef avr_memory_t
 * This type is used to represent a memory space.
 * @ingroup memory
 */
typedef struct avr_memory_t
{
...
#	endif
#ifdef AVR_MEM_SPY
	avr_mem_spy_t spy_fun;	/** spy function */
 ...
 */
static void avr_mem_default_spy(avr_memory_t *mem, avr_address_t addr, avr_size_t size, avr_access_t access, void *data) {
}
#endif
...
avr_memory_t* avr_mem_new(void) {
	...
	/* initialize spy */
#	ifdef AVR_MEM_SPY
		mem->spy_fun = avr_mem_default_spy;
		mem->spy_data = 0;
#	endif

	return (avr_memory_t*) mem;
}
 ...
 */
static page_entry_t* mem_get_page(memory_64_t* mem, avr_address_t addr)
{
    uint32_t      hash1;
    page_entry_t* tmp;
    page_entry_t* entry;
...
avr_memory_t* avr_mem_copy(avr_memory_t* memory)
{
    ...
    memory_64_t* target = (memory_64_t*)avr_mem_new();
 ...
 * @ingroup memory
 */
void avr_mem_write(avr_memory_t* memory, avr_address_t address, void* buffer, size_t size)
{
    assert(size > 0);
	int i;
...
#	ifdef AVR_MEM_SPY
    	mem->spy_fun(mem, address, size, avr_access_write, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
void avr_mem_read(avr_memory_t *memory, avr_address_t address, void *buffer, size_t size)//////////////////////////////
{
    assert(size > 0);
	int i;
    uint32_t      offset = FMOD(address, MEM_PAGE_SIZE);
...
#	ifdef AVR_MEM_SPY
    	mem->spy_fun(mem, address, size, avr_access_read, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
uint8_t avr_mem_read8(avr_memory_t *mem, avr_address_t address)
{
    page_entry_t* pte    = mem_get_page(mem, address);
    avr_address_t offset = FMOD(address, MEM_PAGE_SIZE);
    uint8_t r;
...
#	ifdef AVR_MEM_SPY
    	mem->spy_fun(mem, address, sizeof(r), avr_access_read, mem->spy_data);
#	endif
    return r;
}
 ...
 * @ingroup memory
 */
uint16_t avr_mem_read16(avr_memory_t *mem, avr_address_t address) {
...
#   endif
    ...
    if(!((offset & 0x1) | ((offset + 1) & MEM_PAGE_SIZE)))
        ...
        avr_mem_read(mem, address, val.bytes, 2);
        r = val.half;
    }
 ...
 * @ingroup memory
 */
uint32_t avr_mem_read32(avr_memory_t *mem, avr_address_t address)
{
...
#   endif
    ...
    else
    {
		...
		avr_mem_read(mem, address, val.bytes, 4);
        r = val.word;
    }
 ...
 * @ingroup memory
 */
uint64_t avr_mem_read64(avr_memory_t *mem, avr_address_t address) {
...
#   if HOST_ENDIANNESS != TARGET_ENDIANNESS
    ...
    else
    {
        ...
        avr_mem_read(mem, address, val.bytes, 8);
        r = val.dword;
    }
 ...
 * @ingroup memory
 */
float avr_mem_readf(avr_memory_t *mem, avr_address_t address)
{
	union {
		uint32_t i;
		float f;
	} val;
    val.i = avr_mem_read32(mem, address);
	return val.f;
}
 ...
 * @ingroup memory
 */
double avr_mem_readd(avr_memory_t *mem, avr_address_t address)
{
	union {
		uint64_t i;
		double f;
	} val;
    val.i = avr_mem_read64(mem, address);
	return val.f;
}
 ...
 * @ingroup memory
 */
long double avr_mem_readld(avr_memory_t *memory, avr_address_t address)
{
 ...
 * @ingroup memory
 */
void avr_mem_write8(avr_memory_t* mem, avr_address_t address, uint8_t val)
{
    avr_address_t offset;
    page_entry_t*   pte;
    pte    = mem_get_page(mem, address);
...
#	ifdef AVR_MEM_SPY
    	mem->spy_fun(mem, address, sizeof(val), avr_access_write, mem->spy_data);
#	endif
}
 ...
 * @ingroup memory
 */
void avr_mem_write16(avr_memory_t* mem, avr_address_t address, uint16_t val)
{
    avr_address_t offset;
    uint16_t* q;
...
# endif
    ...
    if(!((offset & 0x1) | ((offset + 1) & MEM_PAGE_SIZE)))
        *q = val;
		...
		avr_mem_write(mem, address, p->bytes, 2);
	}
 ...
 * @ingroup memory
 */
void avr_mem_write32(avr_memory_t* mem, avr_address_t address, uint32_t val) {
	uint32_t* q;
    avr_address_t offset;
    page_entry_t*   pte;
...
#   endif
    ...
    if(!((offset & 0x3) | ((offset + 3) & MEM_PAGE_SIZE)))
        *q = val;
        ...
        avr_mem_write(mem, address, p->bytes, 4);
	}
 ...
 * @ingroup memory
 */
void avr_mem_write64(avr_memory_t *mem, avr_address_t address, uint64_t val)
{
    avr_address_t offset;
    uint64_t* q;
...
#   endif
    ...
    if(!((offset & 0x7) | ((offset + 7) & MEM_PAGE_SIZE)))
        *q = val;
        ...
        avr_mem_write(mem, address, p->bytes, 8);
	}
 ...
 * @ingroup memory
 */
void avr_mem_writef(avr_memory_t *memory, avr_address_t address, float val)
{
	union {
		uint32_t i;
		float f;
	...
	v.f = val;
    avr_mem_write32(memory, address, v.i);
}
 ...
 * @ingroup memory
 */
void avr_mem_writed(avr_memory_t *memory, avr_address_t address, double val)
{
	union {
		uint64_t i;
		double f;
	...
	v.f = val;
    avr_mem_write64(memory, address, v.i);
}
 ...
 * @ingroup memory
 */
void avr_mem_writeld(avr_memory_t *memory, avr_address_t address, long double val)
{
	assertp(0, "not implemented");
}
