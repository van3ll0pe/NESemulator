...
#	endif
	...
	/* allocate handler */
	TRACE;
	loader = (avr_loader_t *)malloc(sizeof(avr_loader_t));
	if(loader == NULL) {
		errno = ENOMEM;
		close(elf);
		//!!DEBUG!!
...
{
	assert(loader);
	/*avr_loader_t *backup = malloc(sizeof(avr_loader_t));*/
...
void avr_loader_load(avr_loader_t *loader, avr_platform_t *pf) {
	...
	avr_memory_t *memory = avr_get_memory(pf, AVR_MAIN_MEMORY);
    ...
    while(ptr_tex != NULL)
    {
        TRACE;
        if(ptr_tex->size)
			avr_mem_write(memory, ptr_tex->address, ptr_tex->bytes, ptr_tex->size);
    ...
    while(ptr != NULL)
    {
		TRACE;
		if(ptr->size)
			avr_mem_write(memory, ptr->address, ptr->bytes, ptr->size);
		ptr = ptr->next;
	}

	/* initialize BSS part */
#	ifdef AVR_NOBITS_INIT
		for(i = 0; i < loader->Tables.sechdr_tbl_size; i++) {
			...
			if(sec->sh_type == SHT_NOBITS) {
				...
				for(j = 0; j < sec->sh_size; j += sizeof(uint32_t))
					avr_mem_write32(memory, sec->sh_addr + j, 0);
			}
		}
#	endif
}
	...
	case SHT_PROGBITS:
		if(s->sh_flags & SHF_ALLOC) {
			if(s->sh_flags & SHF_EXECINSTR)
				data->type = AVR_LOADER_SECT_TEXT;
			else
				data->type = AVR_LOADER_SECT_DATA;
		}
		else
			data->type = AVR_LOADER_SECT_BSS;
		break;
	default:
		data->type = AVR_LOADER_SECT_UNKNOWN;
		break;
	}
...
void avr_loader_sym(avr_loader_t *loader, int sym, avr_loader_sym_t *data) {
	assert(sym < avr_loader_count_syms(loader));
	Elf32_Sym *s;
	...
	case STB_LOCAL: data->bind = AVR_LOADER_LOCAL; break;
	case STB_GLOBAL: data->bind = AVR_LOADER_GLOBAL; break;
	case STB_WEAK: data->bind = AVR_LOADER_WEAK; break;
	default: data->bind = AVR_LOADER_NO_BINDING; break;
	}

	/* get the type */
	switch(ELF32_ST_TYPE(s->st_info)) {
	case STT_FUNC: data->type = AVR_LOADER_SYM_CODE; break;
	case STT_OBJECT: data->type = AVR_LOADER_SYM_DATA; break;
	default: data->type = AVR_LOADER_SYM_NO_TYPE; break;
	}
}
...
avr_address_t avr_brk_init(avr_loader_t *loader)
{
	...
	among all loadable segments */
     ...
     for (i = 0; i < n; i++)
     {
	     ...
	     {
		     avr_address_t new_brk = seg_tbl[i].p_vaddr + seg_tbl[i].p_memsz;
		     if (new_brk > brk_init)
			     brk_init = new_brk;
	     }
     }
