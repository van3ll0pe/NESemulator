...
#	endif
	...
	/* allocate handler */
	TRACE;
	loader = (mos6502_loader_t *)malloc(sizeof(mos6502_loader_t));
	if(loader == NULL) {
		errno = ENOMEM;
		close(elf);
		//!!DEBUG!!
...
{
	assert(loader);
	/*mos6502_loader_t *backup = malloc(sizeof(mos6502_loader_t));*/
...
void mos6502_loader_load(mos6502_loader_t *loader, mos6502_platform_t *pf) {
	...
	mos6502_memory_t *memory = mos6502_get_memory(pf, MOS6502_MAIN_MEMORY);
    ...
    while(ptr_tex != NULL)
    {
        TRACE;
        if(ptr_tex->size)
			mos6502_mem_write(memory, ptr_tex->address, ptr_tex->bytes, ptr_tex->size);
    ...
    while(ptr != NULL)
    {
		TRACE;
		if(ptr->size)
			mos6502_mem_write(memory, ptr->address, ptr->bytes, ptr->size);
		ptr = ptr->next;
	}

	/* initialize BSS part */
#	ifdef MOS6502_NOBITS_INIT
		for(i = 0; i < loader->Tables.sechdr_tbl_size; i++) {
			...
			if(sec->sh_type == SHT_NOBITS) {
				...
				for(j = 0; j < sec->sh_size; j += sizeof(uint32_t))
					mos6502_mem_write32(memory, sec->sh_addr + j, 0);
			}
		}
#	endif
}
	...
	case SHT_PROGBITS:
		if(s->sh_flags & SHF_ALLOC) {
			if(s->sh_flags & SHF_EXECINSTR)
				data->type = MOS6502_LOADER_SECT_TEXT;
			else
				data->type = MOS6502_LOADER_SECT_DATA;
		}
		else
			data->type = MOS6502_LOADER_SECT_BSS;
		break;
	default:
		data->type = MOS6502_LOADER_SECT_UNKNOWN;
		break;
	}
...
void mos6502_loader_sym(mos6502_loader_t *loader, int sym, mos6502_loader_sym_t *data) {
	assert(sym < mos6502_loader_count_syms(loader));
	Elf32_Sym *s;
	...
	case STB_LOCAL: data->bind = MOS6502_LOADER_LOCAL; break;
	case STB_GLOBAL: data->bind = MOS6502_LOADER_GLOBAL; break;
	case STB_WEAK: data->bind = MOS6502_LOADER_WEAK; break;
	default: data->bind = MOS6502_LOADER_NO_BINDING; break;
	}

	/* get the type */
	switch(ELF32_ST_TYPE(s->st_info)) {
	case STT_FUNC: data->type = MOS6502_LOADER_SYM_CODE; break;
	case STT_OBJECT: data->type = MOS6502_LOADER_SYM_DATA; break;
	default: data->type = MOS6502_LOADER_SYM_NO_TYPE; break;
	}
}
...
mos6502_address_t mos6502_brk_init(mos6502_loader_t *loader)
{
	...
	among all loadable segments */
     ...
     for (i = 0; i < n; i++)
     {
	     ...
	     {
		     mos6502_address_t new_brk = seg_tbl[i].p_vaddr + seg_tbl[i].p_memsz;
		     if (new_brk > brk_init)
			     brk_init = new_brk;
	     }
     }
