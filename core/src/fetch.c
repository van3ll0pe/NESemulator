...
/**
 *  initialization and destruction of mos6502_fetch_t object
 */
...
{
	mos6502_fetch_t *res = malloc(sizeof(mos6502_fetch_t));
	if (res == NULL)
		mos6502_error("not enough memory to create a mos6502_fetch_t object"); /* I assume error handling will remain the same, we use mos6502_error instead of iss_error ? */
	res->mem = mos6502_get_memory(pf, MOS6502_MAIN_MEMORY);
...
void mos6502_delete_fetch(mos6502_fetch_t *fetch)
{
	if (fetch == NULL)
		...
		mos6502_error("cannot delete an NULL mos6502_fetch_t object");
	free(fetch);
	number_of_fetch_objects--;
...
mos6502_ident_t mos6502_fetch(mos6502_fetch_t *fetch, mos6502_address_t address, mask_t *code) {
	...
	Table_Decodage_CISC *ptr2 =mos6502_table;
	...
	do {
		...
		while (get_mask_length(code) < get_mask_length(ptr2->mask)) {
			uint8_t buff[4];
			uint32_t word;
			mos6502_mem_read(fetch->mem, address + (get_mask_length(code) >> 3), buff, 4);
#			ifdef MOS6502_ORDER_BYTES_CISC
				MOS6502_ORDER_BYTES_CISC;
...
#			endif
	...
	return (mos6502_ident_t)ptr->table[value].ptr;
}
